...
class HuggingFace:
    ...
    def topk_first(
        ...
        """
        Compute top-k next-token distribution at the first decoding step (t=1),
        WITHOUT generating text. Returns per-prompt list of top-k items.

        Return:
          [
            [ {"rank":1, "token_id":..., "token_str":..., "token_text":..., "prob":..., "logit":...}, ... topk ],
            ...
          ]
        """
        batch_size = len(full_prompts_list)
        ...
        # forward once
        out = self.model(**inputs)  # CausalLMOutput
        logits = out.logits  # [B, L, V]

        # last non-pad position per sample
        if "attention_mask" in inputs:
            last_pos = inputs["attention_mask"].sum(dim=1) - 1  # [B]
        else:
            last_pos = torch.full((logits.size(0),), logits.size(1) - 1,
        ...
        top_probs, top_ids = torch.topk(probs, k=topk, dim=-1)  # [B, K]
        ...
        for b in range(batch_size):
            ...
            for r in range(topk):
                ...
                p = float(top_probs[b, r].item())
                logit = float(next_logits[b, tid].item())
                tok_str = self.tokenizer.convert_ids_to_tokens(tid)
                tok_text = self.tokenizer.decode([tid], clean_up_tokenization_spaces=False)
                items.append({
                    "rank": r + 1,
                    "token_id": tid,
                    "token_str": tok_str,
